...
export function Mint({ onSuccess }) {
    ...
    const checkIndex = async () => {
        ...
        const plugActor = await window.ic.plug.createActor({ canisterId, interfaceFactory: dip721v2_idl });
    ...
    const mintNFT = async () => {
        ...
        await window.ic.plug.createAgent({ whitelist });
        const plugActor = await window.ic.plug.createActor({ canisterId, interfaceFactory: dip721v2_idl });
        const storageActor = await window.ic.plug.createActor({ canisterId: cipherAssets, interfaceFactory: assetIDL.idlFactory });
        const image = await sceneService.getScreenShot();
        const model = await sceneService.getModelFromScene("glb");
        const { hair, face, tops, arms, shoes, legs } = sceneService.getTraits();
        ...
        }
        {
            ...
            const promises = [];
            const chunkSize = 700000;
            for (let start = 0; start < model.size; start += chunkSize) {
                const chunk = model.slice(start, start + chunkSize);
                promises.push(createChunkDefault({
                    batch_id,
                    chunk
                }));
            ...
            const storageResponse = await storageActor.commit_batch({
                batch_id,
                operations: [{ 'CreateAsset': { key: 'model', content_type: model.type } }],
            });
    ...
    return (_jsx(_Fragment, { children: _jsxs("div", { className: 'buttonContainer', children: [_jsx("h6", { children: "Minting Dip721v2:" }), _jsx("button", { id: 'mintNFT', onClick: mintNFT, className: 'mintButton', children: "Mint" })] }) }));
}
